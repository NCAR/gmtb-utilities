...
# Directory where forecast output from pcp-combine is located
pcp_combine_fcst_dir="${base}/gf/pcp_combine"

# Directory where forecast output from CDO/NCO commands is located
pcp_combine_fcst_proc="${pcp_combine_fcst_dir}/time"

# Input directory for hovmoller plotting where observation or forecast file is located
in_obs_dir="${pcp_combine_obs_proc}/cat/"
in_fcst_dir="${pcp_combine_fcst_dir}/cat/"
...
while [[ ${init_cur_ut} -le ${init_end_ut} ]]; do
  ...
  for fcst_time in ${fcst_time_list}; do
    ...
    obs_file_time=${pcp_combine_obs_proc}/time/${vyyyymmdd}/${obtype}_${vyyyymmdd}_${accum_time}h.nc
    ...
    mkdir -p ${pcp_combine_fcst_dir}/time

    # Create fcst file to be created as output from ncap command
    fcst_file_time=${pcp_combine_fcst_dir}/time/${init}_f${fcst_time}_${accum_time}h.nc
...
obs_file_cat=${pcp_combine_obs_proc}/cat/${obtype}_APCP${accum_time}_${init_time_start}-${init_time_end}_f${fcst_time}v${vhh}Z.nc

# Run cdo copy command on observation files
cdo_obs_command="cdo copy ${pcp_combine_obs_proc}/time/*/*${accum_time}h.nc ${obs_file_cat}"
echo "cdo copy ${pcp_combine_obs_proc}/time/*/*${accum_time}h.nc ${obs_file_cat}"
${cdo_obs_command}
...
fcst_file_cat=${pcp_combine_fcst_dir}/cat/APCP${accum_time}_${init_time_start}-${init_time_end}_f${fcst_time}v${vhh}Z.nc

# Run cdo copy command on forecast files
cdo_fcst_command="cdo copy ${pcp_combine_fcst_dir}/time/*${accum_time}h.nc ${fcst_file_cat}"
echo "cdo copy ${pcp_combine_fcst_dir}/time/*${accum_time}h.nc ${fcst_file_cat}"
${cdo_fcst_command}

#### Hovmoller plotting #### 

# Make output directory where forecast or observation hovmoller plot will be written 
mkdir -p ${out_dir}

# Name of input obs and forecast file to be plotted
in_obs_file=${obs_file_cat}
in_fcst_file=${fcst_file_cat}

# Name of output file
obs_hovmoller_plot=`basename ${in_obs_file} | cut -d"." -f1`
fcst_hovmoller_plot=`basename ${in_fcst_file} | cut -d"." -f1`

ncl "in_file=\"${in_obs_file}\"" "out_dir=\"${out_dir}\"" "out_file=\"${obs_hovmoller_plot}\"" "precip_var=\"${precip_var}\"" < ${ncl_script}

ncl "in_file=\"${in_fcst_file}\"" "out_dir=\"${out_dir}\"" "out_file=\"${fcst_hovmoller_plot}\"" "precip_var=\"${precip_var}\"" < ${ncl_script}
